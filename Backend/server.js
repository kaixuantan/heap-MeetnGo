        ...
        axios
            ...
            .then((response) => {
                ...
                const events = response.data.items.map((event) => {
                    ...
                    const startdatetime =
                        Object.keys(start).length > 1 ? start : "Whole Day";
                    ...
                    if (startdatetime && startdatetime.dateTime) {
                        ...
                        StartDate = dayjs(startdatetime.dateTime)
                            .utc()
                            .tz(userTimezone)
                            .format("YYYY-MM-DD");
                        const eventDuration = dayjs(event.end.dateTime).diff(
                            dayjs(startdatetime.dateTime),
                            "day"
                        );
                        EndDate = dayjs(StartDate)
                            .add(eventDuration, "day")
                            .format("YYYY-MM-DD");
                    } else {
...
app.get("/filter", async (req, res) => {
    ...
    if (requestedCategory === "all") {
        axios
            .get(`http://localhost:3000/event/api/${email}`)
            .then(async (response) => {
            ...
            .then(async (response) => {
                ...
                if (data) {
                    ...
                    other_events = all_events.filter((itemA) => {
                        return !my_events.some(
                            (itemB) =>
                                itemB.EventName === itemA.EventName &&
                                itemB.EventID === itemA.EventID
                        );
                    });
                }
